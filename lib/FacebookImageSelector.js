...
FacebookImageSelector = React.createClass({
	...
	componentDidMount: function () {
		...
		this.loadStylesheet('https://cdn.rawgit.com/vinitj/facebook-image-selector/cf787b41b59f7e67e7147b3678bb364292d2d17f/css/facebookImageSelector.css');
		if (!window.FB && appId) this.setupFacebook(appId);
		if (window.FB && !this.props.clickText && this.props.open) this.handleFacebookImageSelector();
  },

  componentWillReceiveProps: function(nextProps) {
  	if (!this.state.showOverlay && nextProps.open) this.handleFacebookImageSelector();
  	if (this.state.showOverlay && !nextProps.open) this.closeOverlay();
  },

  setupFacebook: function(appId) {
  ...
  componentWillUnmount: function () {
	...
	statusChangeCallback: function(response) {
		if (response.status === 'connected') {
			...
			this.props.onError(ErrorMessages.notLoggedin);
		}
	},
	...
	showError : function (error) {
		this.props.onError(error || ErrorMessages["default"]);
			...
			return (
				React.createElement("div", {className: "facebookImageSelector"}, 
					(state.showOverlay) ?  React.createElement(ImageLoader, {data: (state.albumsLoaded) ? state.albumDataLoaded : state.photoDataLoaded, 
						...
						(this.props.clickText) ? React.createElement("div", {className: "fbImageSelectorText", onClick: this.handleFacebookImageSelector}, this.props.clickText) : ''
				)
			);
		} else {
			return (React.createElement("div", {className: "facebookImageSelector noAppId"}, ErrorMessages.noAppId));
		}
	}
});
			...
			React.createElement("div", {className: "overlay"}, 
				React.createElement("div", {className: "content"}, 
					...
					React.createElement("section", null, 
						...
						(props.isError) ? React.createElement("div", {className: "block-container-error"}, props.customError) :
								(allAlbums && allAlbums.length > 0) ? React.createElement("div", {className: "block-container", ref: "dataNode"}, allAlbums) :
									React.createElement("div", {className: "block-container-error"}, ErrorMessages["noPhoto"]), 
					...
					React.createElement("footer", null, 
						...
						(!props.type && allAlbums && allAlbums.length > 0) ? React.createElement("div", {className: "selector", onClick: self.okSelector}, "OK") : ''
					)
				), 
